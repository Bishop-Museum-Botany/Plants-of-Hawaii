...
finalDF$isCultivated <- gsub(-1, "", finalDF$isCultivated)
finalDF$isCultivated <- gsub(0, "", finalDF$isCultivated)
finalDF$isCultivated <- gsub(1, "Cultivated", finalDF$isCultivated)
finalDF$isCultivated <- gsub("Yes", "Cultivated", finalDF$isCultivated)

finalDF_cult <- subset(finalDF, isCultivated == "Cultivated")
finalDF_nocult <- subset(finalDF, isCultivated != "Cultivated")
```

```{r}
#look for "cultivate"

finalDF_nocult$cultivate <- ifelse(grepl("cultivate", finalDF_nocult$habitat, ignore.case = TRUE) | 
                           grepl("cultivate", finalDF_nocult$occurrenceRemarks, ignore.case = TRUE) |
                           grepl("cultivate", finalDF_nocult$associatedTaxa, ignore.case = TRUE) |
                           grepl("cultivate", finalDF_nocult$locality, ignore.case = TRUE) , "Cultivated", NA)
...
finalDF_nocult <- finalDF_nocult %>% dplyr::select(Allfields)
finalDF_cult <- finalDF_cult %>% dplyr::select(Allfields)
