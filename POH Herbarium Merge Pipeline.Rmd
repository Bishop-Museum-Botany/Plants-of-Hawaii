...
```{r}
BISH <- merge(POH_taxon_2, BISH, by = "acceptedNameUsageID", all.x = FALSE, all.y = FALSE) %>% dplyr::rename(scientificName = scientificName.x)
```
...
Second merge to assign an accepted scientific name
```{r}
PTBG <- merge(POH_taxon_2, PTBG, by = "acceptedNameUsageID", all.x = FALSE, all.y = FALSE) %>% dplyr::rename(scientificName = scientificName.x)
```

```{r}
subset(PTBG, is.na(PTBG$TaxonID))
```
...
Second merge to assign an accepted scientific name
```{r}
GBIF_pres_spec <- merge(POH_taxon_2, GBIF_pres_spec, by = "acceptedNameUsageID", all.x = FALSE, all.y = FALSE) %>% dplyr::rename(scientificName = scientificName.x, Year = year)
```
```{r}
GBIF_pres_spec
```
...
```{r}
US <- merge(POH_taxon_2, US, by = "acceptedNameUsageID", all.x = FALSE, all.y = FALSE) %>% dplyr::rename(scientificName = scientificName.x)
```
...
Second merge to assign an accepted scientific name
```{r}
idigbio <- merge(POH_taxon_2, idigbio, by = "acceptedNameUsageID", all.x = FALSE, all.y = FALSE) %>% dplyr::rename(scientificName = scientificName.x)
```
```{r}
...
Just doing some checks because it appears that excel is reading the data strangely



```{r}
head(sort(unique((finalDF$taxonID))))
```
```{r}
head(sort(unique((finalDF$occurrenceID))))
```
```{r}
taxonIDissue <- subset(finalDF, is.na(finalDF$taxonID))
taxonIDissue
```
```{r}
OccIDissue <- subset(finalDF, is.na(finalDF$occurrenceID))
OccIDissue
```

```{r}
length(unique(taxonIDissue$acceptedNameUsageID))
```
