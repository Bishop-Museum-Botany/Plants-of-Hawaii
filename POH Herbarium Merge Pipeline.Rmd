...
```{r}
#Herbarium Data                                                                                                         
BISH <- read.csv("dbo_Specimens1a.csv", header=T, sep=',', stringsAsFactors=F) %>% dplyr::select(-c(Date2))
BISH_old <- read.csv("BISH2018.csv", header=T, sep=',', stringsAsFactors=F) #%>% dplyr::select(-c(eventDate))
#PTBG <- read.table("NTBG_HerbariumSearchExport_20201221.txt", header=T, sep='\t', quote="", fill = TRUE, stringsAsFactors=F) %>% dplyr::select(-c(COLDATE))
...
### DANGER!!!! Having trouble with excel formatting dates, so I've extracted in the proper format and rebinded
This is not ideal, and needs to be dealt with. If not careful, you could mix up specimens and dates. Not good.

```{r}
Date2 <- read.csv("BISH_Date2.csv", header=T, sep=',', stringsAsFactors=F)
eventDate <- read.csv("BISH_old_eventDate.csv", header=T, sep=',', stringsAsFactors=F)
#COLDATE <- read.csv("PTBG_COLDATE.csv", header=T, sep=',', stringsAsFactors=F)
#Date.Collected <- read.csv("US_Collection.Date.csv", header=T, sep=',', stringsAsFactors=F)
Date.Collected <- read.table("US_Collection.Date.txt", header = TRUE, sep = "/") #, dec = ".")
```
```{r}
BISH <- cbind(BISH, Date2)
BISH_old <- cbind(BISH_old, eventDate)
#PTBG <- cbind(PTBG, COLDATE)
```
...
```{r}
nrow(BISH) + nrow(BISH_old) + nrow(PTBG) + nrow(US) + nrow(GBIF_pres_spec)
```
```{r}
nrow(BISH)
```
...
# TEMPORARY OLD BISH Herbarium Data
## 1. BISH_old- Merge with POH table

Merging with taxon names and ID #s

How many species won't merge (ie. have no scientific name associated with a BISH_taxonID?)?

```{r}
# get rid of blank species names
BISH_old <- subset(BISH_old, BISH_old$taxon.scientificName != "")
```


```{r}
temp <- merge(POH_taxon_1, BISH_old, by.y = "taxon.scientificName", by.x = "scientificName", all.x = FALSE, all.y = TRUE)
...
#BISH_old <- BISH_old %>% dplyr::select(-c(genus, family))
BISH_old<- merge(POH_taxon_1, BISH_old, by.y = "taxon.scientificName", by.x = "scientificName", all.x = FALSE, all.y = FALSE)
# but still want to export mismatches
write.csv(temp, file = "BISH_oldtaxa_Not_in_POH_table_12102020.csv", row.names = FALSE)
```

Second merge to assign an accepted scientific name
```{r}
BISH_old <- merge(POH_taxon_2, BISH_old, by = "acceptedNameUsageID", all.x = FALSE, all.y = TRUE) %>% dplyr::rename(scientificName = scientificName.x)
```


```{r}
BISH_old <- BISH_old %>% dplyr::select(dplyr::any_of(Allfields))
```


```{r}
BISH <- dplyr::bind_rows(mutate_all(BISH, as.character), mutate_all(BISH_old, as.character))
```
...
```{r}
combinedDF <- dplyr::bind_rows(mutate_all(combinedDF, as.character), mutate_all(BISH, as.character), mutate_all(PTBG, as.character), mutate_all(US, as.character), mutate_all(GBIF_pres_spec, as.character))
```
```{r}
nrow(combinedDF)
```
...
sort(unique(deduped_combinedDF$island)
```
```{r}
...
```{r}
temp <- finalDF %>% find_duplicates(recordedBy, recordNumber)
...
#finalDF <-distinct(finalDF, locality, recordedBy, recordNumber, scientificName, eventDate, year, .keep_all = TRUE)
```
```{r}
nrow(finalDF)
```
...
write.table(finalDF, file = "All_herbaria_COMPILED_2020-12-21.txt", sep = "\t", row.names = FALSE)
saveRDS(finalDF, file= "All_herbaria_COMPILED_2020-12-21.rds")
```



```{r}
finalDF_KBedits<- read.csv("All_herbaria_COMPILED_2020-12-17_KBedit.csv", header=T, sep=',', stringsAsFactors=F)
```
```{r}
Location <- finalDF_KBedits %>% dplyr::select(all_of(Locationfields))
Occurrence <- finalDF_KBedits %>% dplyr::select(all_of(Occurrencefields))

write.csv(Location, file = "All_herbaria_COMPILED_LOCATION2020-12-17.csv", row.names = FALSE)
write.csv(Occurrence, file = "All_herbaria_COMPILED_OCCURRENCE2020-12-17.csv", row.names = FALSE)
