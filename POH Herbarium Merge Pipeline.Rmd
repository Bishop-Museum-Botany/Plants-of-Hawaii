...
## Formatting for BISH database
```{r}
unique(finalDF$locationID)
```
...
locationIDnew <- paste0(locationiddf$first, "-" ,locationiddf$second, "-", 4, locationiddf$third, "-", locationiddf$fourth, "-", locationiddf$fifth)   
head(locationIDnew, 25)
```
```{r}
finalDF$locationID <- locationIDnew
finalDF
```
...
occurrenceIDnew <- paste0(occurrenceiddf$first, "-" ,occurrenceiddf$second, "-", 4, occurrenceiddf$third, "-", occurrenceiddf$fourth, "-", occurrenceiddf$fifth)   
head(occurrenceIDnew, 25)
```
```{r}
finalDF$occurrenceID <- occurrenceIDnew
finalDF
```

```{r}
#forwrite <- gsub("\t", "", finalDF)
write.csv(finalDF, file = "All_herbaria_COMPILED_2020-01-21.csv", row.names = FALSE)
saveRDS(finalDF, file= "All_herbaria_COMPILED_2020-01-21.rds")
```
...
Occurrencefields <- c("occurrenceID", "locationID", "taxonID", "basisOfRecord", "catalogNumber", "recordNumber", "recordedBy", "individualCount", "organismQuantityProperty", "organismQuantityTypeProperty", "sex", "lifeStage", "reproductiveCondition", "behavior", "establishmentMeans", "occurrenceStatus", "preparations", "disposition", "associatedMedia", "associatedReferences", "associatedTaxa", "otherCatalogNumbers", "occurrenceRemarks", "fieldNumber", "eventDate", "eventTime", "habitat", "samplingProtocol", "fieldNotes", "watershedCount", "isCultivated", "source", "sourceIdentifier", "sourceURL")
```
...
```{r}
#forwrite <- gsub("\t", "", finalDF)
write.csv(OccurrenceDF, file = "Occurrence_COMPILED_2020-01-21.csv", row.names = FALSE)
write.csv(LocationDF, file= "Location_COMPILED_2020-01-21.rds", row.names = FALSE)
```
