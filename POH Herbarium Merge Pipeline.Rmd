...
8) Assigns a cultivated, naturalized or unknown status
...
5) Clean collector numbers so we can find duplicates easier
6) PRIORITY! Reconcile discarded occurrences with POH taxon list - do some need to be added to the list?
7) PRIORITY! lubridate is giving unusual results for some formats, at least for US data
...
```{r}
deduped_combinedDF <-distinct(combinedDF, locality, eventDate, recordNumber, scientificName, .keep_all = TRUE)
```
```{r}
