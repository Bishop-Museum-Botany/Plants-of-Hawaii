...
```
```{r}
temp <- merge(POH_taxon_1, GBIF_obs,  by.x = "scientificName", by.y = "scientificName_clean", all.x = FALSE, all.y = TRUE)
temp <- subset(temp, is.na(temp$acceptedNameUsageID))
occ <- nrow(temp)
tax <- length(unique(temp$scientificName))
# but still want to export mismatches
write.csv(temp, file = "GBIFOBStaxa_Not_in_POH_table_01262021.csv", row.names = FALSE)
print(paste0("lost ", occ , " occurrences represented by ", tax, " unique values in the taxa names field"))
```
```{r}
GBIF_obs<- merge(POH_taxon_1, GBIF_obs,  by.x = "scientificName", by.y = "scientificName_clean", all.x = FALSE, all.y = FALSE)
```

Second merge to assign an accepted scientific name
```{r}
GBIF_obs <- merge(POH_taxon_2, GBIF_obs, by = "acceptedNameUsageID", all.x = FALSE, all.y = TRUE) %>% dplyr::rename(scientificName = scientificName.x, Year = year)
```
...
```
```{r}
POHImages<- merge(POH_taxon_1, POHImages,  by = "taxonID", all.x = FALSE, all.y = FALSE)
nrow(POHImages)
```

Second merge to assign an accepted scientific name
```{r}
POHImages <- merge(POH_taxon_2, POHImages, by = "acceptedNameUsageID", all.x = FALSE, all.y = TRUE) %>% dplyr::rename(scientificName = scientificName.x)
nrow(POHImages)
```
...
```{r}
combinedDF <- dplyr::bind_rows(mutate_all(combinedDF, as.character), mutate_all(GBIF_obs, as.character), mutate_all(POHImages, as.character))
```
```{r}
nrow(combinedDF)
```
...
write.csv(finalDF, file = "All_observations_COMPILED_2020-01-21.csv", row.names = FALSE)
saveRDS(finalDF, file= "All_observations_COMPILED_2020-01-21.rds")
```
...
```{r}
#forwrite <- gsub("\t", "", finalDF)
write.csv(OccurrenceDF, file = "Obs_Occurrence_COMPILED_2020-01-21.csv", row.names = FALSE)
write.csv(LocationDF, file= "Obs_Location_COMPILED_2020-01-21.csv", row.names = FALSE)
write.csv(forPOH, file= "Obs_forPOH_2020-01-21.csv", row.names = FALSE)
```
